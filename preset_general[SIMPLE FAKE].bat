...
--filter-tcp=%GameFilterTCP% --ipset="%~dp0files\ipset-general.txt" --ipset-exclude="%~dp0files\ipset-exclude.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n4 --dpi-desync-fooling=ts --dpi-desync-fake-tls="%~dp0files\stun.bin" --dpi-desync-fake-tls="%~dp0files\tls_clienthello_www_google_com.bin" --dpi-desync-fake-http="%~dp0files\tls_clienthello_max_ru.bin" --new ^
