...
::ZAPRET DIAGNOSTICS (NEW)
:tcp_diagnostics
chcp 437 > nul
cls
::TCP DIAGNOSTICS (LITE)
netsh interface tcp show global | findstr /i "timestamps" | findstr /i "enabled" > nul
if !errorlevel!==0 (
    echo NO ERROR FOUND.
) else (
    echo ERROR WAS FOUND, I AM FIXING IT.
    netsh interface tcp set global timestamps=enabled > nul 2>&1
    if !errorlevel!==0 (
        echo ERROR HAS BEEN FIXED.
    ) else (
        echo IF YOU WERE UNABLE TO FIX ERROR, PLEASE CONTACT APP DEVELOPER.
    )
)
echo:

:: CLEAR DNS CACHE (LITE)
set /p answer=WANT TO CLEAR YOUR DNS CACHE? (YES/NO): 
